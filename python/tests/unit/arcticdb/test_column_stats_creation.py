...
def test_auto_column_stats_on_write_and_append(lmdb_version_store, any_output_format):
    """Auto column stats: write + append should each produce MINMAX stats for every
    numeric/temporal column without any manual create_column_stats call. String
    columns are silently skipped by the bouncer in build_auto_column_stats."""
    lib = lmdb_version_store
    lib._set_output_format_for_pipeline_tests(any_output_format)
    sym = "test_auto_column_stats_on_write_and_append"

    df_write = pd.DataFrame(
        {
            "int_col": np.array([3, 1, 4, 2], dtype=np.int64),
            "uint_col": np.array([30, 10, 40, 20], dtype=np.uint32),
            "float_col": [30.5, 10.5, 40.5, 20.5],
            "ts_col": pd.to_datetime(["2021-01-01", "2020-01-01", "2021-06-01", "2020-06-01"]),
            "str_col": ["c", "a", "d", "b"],
        },
    ...
    # Numeric and temporal columns get auto-stats; the string column is skipped.
    numeric_cols = ["int_col", "uint_col", "float_col", "ts_col"]
    assert lib.get_column_stats_info(sym) == {c: {"MINMAX"} for c in numeric_cols}

    expected_after_write = index_columns_to_pl(lib, sym).with_columns(
        *[pl.Series(f"v1_MIN({c})", [df_write[c].min()]) for c in numeric_cols],
        *[pl.Series(f"v1_MAX({c})", [df_write[c].max()]) for c in numeric_cols],
    )
    received = lib.read_column_stats(sym)
    received_cols = set(pl.from_arrow(received).columns)
    assert "v1_MIN(str_col)" not in received_cols
    assert "v1_MAX(str_col)" not in received_cols
    assert_stats_equal(received, expected_after_write)

    df_append = pd.DataFrame(
        {
            "int_col": np.array([7, 5, 6], dtype=np.int64),
            "uint_col": np.array([70, 50, 60], dtype=np.uint32),
            "float_col": [70.5, 50.5, 60.5],
            "ts_col": pd.to_datetime(["2023-01-01", "2022-01-01", "2022-06-01"]),
            "str_col": ["g", "e", "f"],
        },
        index=pd.date_range("2000-01-05", periods=3),
    )
    lib.append(sym, df_append)

    assert lib.get_column_stats_info(sym) == {c: {"MINMAX"} for c in numeric_cols}

    expected_after_append = index_columns_to_pl(lib, sym).with_columns(
        *[pl.Series(f"v1_MIN({c})", [df_write[c].min(), df_append[c].min()]) for c in numeric_cols],
        *[pl.Series(f"v1_MAX({c})", [df_write[c].max(), df_append[c].max()]) for c in numeric_cols],
    )
    assert_stats_equal(lib.read_column_stats(sym), expected_after_append)
