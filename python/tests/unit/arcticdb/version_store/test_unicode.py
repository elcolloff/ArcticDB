...
def test_write(lmdb_version_store_tiny_segment, parallel, multi_index):
    ...
    lib.create_column_stats_experimental(symbol, column_stats={trademark: {"MINMAX"}})
    vit = lib.read(symbol)
...
def test_recursively_written_data_with_metadata(lmdb_version_store, batch_read, all_recursive_metastructure_versions):
