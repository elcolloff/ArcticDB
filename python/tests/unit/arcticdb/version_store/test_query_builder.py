...
def test_querybuilder_date_range_then_filter(
    ...
    lib.write(symbol, df)
    lib.create_column_stats_experimental(symbol, {"col1": {"MINMAX"}, "col2": {"MINMAX"}})
...
def test_querybuilder_date_range_then_filter_then_resample(
    ...
    lib.write(symbol, df)
    lib.create_column_stats_experimental(symbol, {"filter_col": {"MINMAX"}, "agg_col": {"MINMAX"}})
...
def test_querybuilder_filter_then_date_range(
    ...
    df = pd.DataFrame({"col": np.arange(1, 11)}, index=pd.date_range("2024-01-01", periods=10))
    lib.write(symbol, df)
    lib.create_column_stats_experimental(symbol, {"col": {"MINMAX"}})
    q = QueryBuilder()
...
def test_querybuilder_groupby_then_filter(
    ...
    lib.write(symbol, df)
    lib.create_column_stats_experimental(symbol, {"col2": {"MINMAX"}})
    q = QueryBuilder()
    q = q.groupby("col1").agg({"col2": "sum"})
